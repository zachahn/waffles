...
With `--named`, each line from a script file or `--stdin` must be a named task of the form `name: command`, where `name` matches `[a-z0-9_-]+`. The name is used as the output label (and `{name}` filename placeholder) instead of the command text. Malformed lines, empty commands, and duplicate names are rejected up front. The flag does not affect `-c`/`--command` commands.
